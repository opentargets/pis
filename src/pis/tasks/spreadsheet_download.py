...
class SpreadsheetDownload(Task):
    ...
    async def run(self) -> Self:
        logger.debug(
            f'exporting to csv gid: {self.spec.gid} from spreadsheet {self.spec.sheet_id}'
        )
        ...
        try:
            ...
            logger.debug(
                f'Using default credentials from environment (project: {project})'
            )
        ...
        if not response.ok:
            error_msg = f'error {response.status_code} getting the spreadsheet reason: {response.reason}. Response body: {response.text}'
